"""VMIP 0.1 Cross-Vendor Tool Marketplace — VinculNet Demo.

Same 9-step scenario as demo.py, but with receipts exchanged over
VinculNet (authenticated WebSocket transport).

Architecture:
  - Vendor A and Vendor B each run a ProtocolPeer with its own VinculRuntime
  - Both runtimes are bootstrapped with identical contract + scope state
  - Tool invocation is local (not Vincul's concern)
  - Success receipts are broadcast over VinculNet to connected peers
  - Receiving peer verifies receipt hash + cross-checks scope/contract hashes
    against its own independent runtime state
"""
...
logger = logging.getLogger("vincul.transport")
...
def _replicate_runtime(source: VinculRuntime) -> VinculRuntime:
    ...
    Deep-copies contracts, scopes, and receipts from the source runtime
    so the replica has matching descriptor_hashes but its own objects.
    """
    replica = VinculRuntime()

    # Replicate contracts (already sealed — just copy them in)
...
async def run_demo() -> None:
    """Run the cross-vendor marketplace demo with VinculNet transport."""

    # Collect receipts received over VinculNet
    vendor_a_received: list[tuple[str, Receipt]] = []
    vendor_b_received: list[tuple[str, Receipt]] = []

    # ==================================================================
    # STEP 1: Vendor Setup — VinculContext + ProtocolPeers
    # ==================================================================
    _section("STEP 1: Vendor Setup (VinculContext + VinculNet Peers)")
    ...
    # Create ProtocolPeers for A and B (each will get its own runtime)
    ...
    # Start VinculNet — Vendor B listens, Vendor A connects
    ...
    # Tool provider uses Vendor A's runtime (where actions are validated)
    ...
    # Now bootstrap both peers with identical state from ctx.runtime
    ...
    # ==================================================================
    # STEP 4: Agent setup + successful invocation + receipt over VinculNet
    # ==================================================================
    _section("STEP 4: Tool Invocation + VinculNet Receipt (should SUCCEED)")
    ...
    # Broadcast success receipt over VinculNet (Vendor A → Vendor B)
    receipt_payload = {"type": "receipt", "receipt": result.receipt.to_dict()}
    sent = await peer_a.peer.send(VENDOR_B_ID, receipt_payload)
    ...
    # ==================================================================
    # STEP 5: Second Invocation + receipt over VinculNet
    # ==================================================================
    _section("STEP 5: Second Invocation + VinculNet Receipt (should SUCCEED)")
    ...
    # Broadcast
    receipt_payload2 = {"type": "receipt", "receipt": result2.receipt.to_dict()}
    await peer_a.peer.send(VENDOR_B_ID, receipt_payload2)
    await asyncio.sleep(0.1)

    print(f"\n  VinculNet: Vendor B total receipts received: {len(vendor_b_received)}")

    # ==================================================================
    # STEP 6: Revocation — VendorB revokes mid scope on its own runtime
    # ==================================================================
    _section("STEP 6: Revocation — VendorB revokes mid scope (cascade)")

    # Vendor B revokes on its own independent runtime
    # The receipt captures the pre-revocation scope_hash
    ...
    # Peer A's runtime still has the scope as active (independent!)
    print(f"\n  Peer A mid scope status: {peer_a.runtime.scopes.get(mid.id).status.value} (still active — independent runtime)")

    # Broadcast revocation receipt over VinculNet (Vendor B → Vendor A)
    # Peer A's scope is still pre-revocation, so scope_hash cross-check passes
    rev_payload = {"type": "receipt", "receipt": rev_receipt.to_dict()}
    await peer_b.peer.send(VENDOR_A_ID, rev_payload)
    await asyncio.sleep(0.1)
    ...
    # Now revoke on Peer A's runtime too (so step 7 fails correctly)
    ctx.revoke_scope(
        scope_id=mid.id,
        contract_id=contract.contract_id,
        initiated_by=VENDOR_B_ID,
    ...
    # Failure receipts are NOT broadcast
    before_count = len(vendor_b_received)
    print(f"\n  VinculNet: Vendor B receipts still: {before_count} (failure NOT broadcast)")
    ...
    print(f"  Total receipts (Vendor A runtime): {len(timeline)}")
    ...
    print("  Built on vincul SDK + VinculNet:")
