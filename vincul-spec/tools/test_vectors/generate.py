#!/usr/bin/env python3
"""
Vincul Protocol — Test Vector Generator v0.2
...
def main():
    print("=" * 68)
    print("Vincul Protocol — Test Vectors v0.2")
    ...
    # ── 1. Scope ──────────────────────────────────────────────
    scope_hash = vincul_hash("scope", SCOPE_VECTOR)
    ...
    contract_hash = vincul_hash("contract", contract_normalized)
    results["contract_hash"] = contract_hash
    print(f"\n── Contract ─────────────────────────────────────────────────")
    print(f"Hash: {contract_hash}")

    # ── 3. Constraints ────────────────────────────────────────
    constraint_top_hash = vincul_hash_constraint("TOP")
    constraint_atom_hash = vincul_hash_constraint("action.params.duration_minutes <= 60")
    ...
    profile_hash = vincul_hash("profile", profile_normalized)
    ...
    delegation_hash = vincul_hash("receipt", delegation_receipt)
    ...
    commitment_hash = vincul_hash("receipt", commitment_receipt)
    ...
    attestation_hash = vincul_hash("receipt", attestation_receipt)
    ...
    ledger_hash = vincul_hash("receipt", ledger_receipt)
    ...
    contract_hash_after = vincul_hash("contract", dissolved_contract)
    ...
    dissolution_hash = vincul_hash("receipt", dissolution_receipt)
    ...
    revocation_hash = vincul_hash("receipt", revocation_receipt)
    ...
    failure_hash = vincul_hash("receipt", failure_receipt)
