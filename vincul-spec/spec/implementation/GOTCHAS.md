# Vincul Protocol — Implementation Gotchas
...
This document collects known implementation pitfalls in Vincul v0.1 — places where the protocol spec is correct and internally consistent, but where the correct behavior is non-obvious and easy to get wrong.
...
**The consequence:** An implementation that skips DAG traversal and trusts stored status may authorize actions under revoked scopes. This is a structural safety violation — exactly the failure mode Vincul's bounded authority model exists to prevent.
