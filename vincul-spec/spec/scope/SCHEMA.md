...
A **Scope** is the minimal, portable representation of **delegated, bounded authority** in Vincul.
...
Vincul defines a closed set of operation types on the impact chain:
...
Vincul enforces structural safety at this level. What `calendar.events:COMMIT` does in the real world is outside Vincul's reasoning boundary.
...
A dot-separated hierarchical path that identifies the resource space this scope governs. Vincul does not interpret namespace semantics. Vincul does enforce that:

- Child namespace coverage ⊆ parent namespace coverage
- Namespace comparison is structural (path prefix or registered mapping)

Namespace registries are implementation-layer concerns. Vincul does not require one.
...
If an implementation requires semantic containment beyond structural prefix, that must be handled by a namespace registry mapping layer. Vincul core uses only this rule.
...
Both `predicate` and `ceiling` are expressions in the Vincul Constraint DSL (defined separately in `spec/dsl/CONSTRAINT.md`). The following invariants govern their relationship:
