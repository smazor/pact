# Vincul Protocol — Constraint DSL
...
The Vincul Constraint DSL defines the language used to express `predicate` and `ceiling` fields in a Scope. It has exactly one job:
...
If an implementation requires any of the above, it must handle them outside the Vincul constraint layer. The DSL does not grow to accommodate implementation needs.
