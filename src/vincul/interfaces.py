"""
vincul.interfaces — Protocol definitions for all store and evaluator boundaries

This module is the dependency firewall. Every module in vincul depends downward
on vincul.types and vincul.interfaces. No module imports concrete classes from
a sibling module — only the runtime composition root does that.

Import graph:
    vincul.types       ← pure data, no deps
    vincul.interfaces  ← depends only on vincul.types
    vincul.contract    ← implements ContractStoreProtocol
    vincul.scopes      ← implements ScopeStoreProtocol
    vincul.receipts    ← implements ReceiptLogProtocol
    vincul.constraints ← implements ConstraintEvaluatorProtocol
    vincul.budget      ← implements BudgetLedgerProtocol
    vincul.validator   ← depends on interfaces only
    vincul.runtime     ← composition root; wires concrete → interfaces
"""
...
from vincul.types import Hash, Timestamp, UUID, ValidationResult
...
class BudgetLedgerProtocol(Protocol):
    ...
    vincul.validator step 7 (BUDGET_EXCEEDED) calls check_available().
    """
