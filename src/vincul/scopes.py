"""
vincul.scopes — Scope DAG store, delegation validation, revocation cascade
...
class Scope:
    """
    A Vincul Scope descriptor.  All fields map directly to SCHEMA.md §6.
    ...
    def compute_hash(self) -> Hash:
        return vincul_hash("scope", self._payload_for_hash())
...
class DelegationValidator:
    ...
    def _check_ceiling_containment(parent: Scope, child: Scope) -> ValidationResult:
        ...
        The constraint DSL evaluator (vincul.constraints) will provide full
        ...
        if child.ceiling == parent.ceiling:
            return ValidationResult.allow()
        # Non-trivial subset check deferred to vincul.constraints
...
class RevocationResult:
    ...
    Consumed by vincul.receipts to produce RevocationReceipt.
    """
    root_scope_id: UUID
