"""
vincul.runtime — Composition root
...
VinculRuntime that exposes high-level orchestration methods.

Each operation follows the two-store model:
  validate → mutate state → emit receipt

Depends on: every other vincul module (composition root)
"""
...
from vincul.scopes import DelegationValidator, RevocationResult, Scope, ScopeStore
from vincul.types import FailureCode, ReceiptKind, Timestamp, UUID
...
class VinculRuntime:
    """
    Composition root for the Vincul Protocol.
    ...
    def delegate(
        ...
        r = delegation_receipt(
            ...
            ceiling_hash=vincul_hash_constraint(child.ceiling),
        )
        self.receipts.append(r)
        return r
